<p align="center"><a href="https://valkyrja.io" target="_blank">
    <img src="https://raw.githubusercontent.com/valkyrjaio/art/refs/heads/26.x/long-banner/orange/default.png" width="100%">
</a></p>

# Valkyrja

[Valkyrja][valkyrja url] is a framework for web and console applications.

Valkyrja (pronounced "Valk-ear-ya") is the Old Norse spelling for Valkyrie, a
mythical creature that would guide warriors to Valhalla after death. In a
similar sense, the Valkyrja framework guides your application to be in a
better state — fast, light, and robust, doing the heavy lifting so you can
focus on what you're building.

Get Started
-----------

Valkyrja is available in multiple languages. Start here:

- **PHP** — [valkyrja-php][valkyrja-php url] _(production — reference
  implementation)_
- **Java** — [valkyrja-java][valkyrja-java url] _(in progress)_

Or start a new application directly:

- **PHP** — [valkyrja-starter-app-php][starter-php url] — use "Use this
  template" or `composer create-project valkyrja/application your-project`
- **Java** — [valkyrja-starter-app-java][starter-java url] — in progress

The Ecosystem
-------------

Surrounding the framework are tools, integrations, and shared conventions:

- **Build tool** — [sindri-php][sindri url] — scaffolds applications and
  generates cache files for faster runtime performance
- **Worker runtimes** — persistent-worker integrations for production:
  [OpenSwoole][openswoole url], [FrankenPHP][frankenphp url],
  [RoadRunner][roadrunner url] _(PHP)_; [Netty][netty url],
  [Jetty][jetty url], [Tomcat][tomcat url] _(Java)_
- **Docker** — [valkyrja-docker-php][docker url] — ready-made containers for
  running Valkyrja applications
- **Architecture** — [architecture][architecture url] — design decisions and
  port planning across all languages
- **Shared tooling** — CI configurations, templates, and conventions live in
  dedicated repos in this organization

Explore the full list in the [repositories tab][org repos url].

Community
---------

- [valkyrja.io][valkyrja url] — project website and documentation
- [Contributing guidelines][contributing url] — how to contribute code,
  tests, and documentation
- [Project vocabulary][vocabulary url] — canonical definitions of Valkyrja
  terms
- [Repository naming conventions][repository naming url] — how repos in this
  organization are named
- [Security policy][security url] — vulnerability disclosure procedure

[valkyrja url]: https://valkyrja.io
[valkyrja-php url]: https://github.com/valkyrjaio/valkyrja-php
[valkyrja-java url]: https://github.com/valkyrjaio/valkyrja-java
[starter-php url]: https://github.com/valkyrjaio/valkyrja-starter-app-php
[starter-java url]: https://github.com/valkyrjaio/valkyrja-starter-app-java
[sindri url]: https://github.com/valkyrjaio/sindri-php
[openswoole url]: https://github.com/valkyrjaio/valkyrja-openswoole-php
[frankenphp url]: https://github.com/valkyrjaio/valkyrja-frankenphp-php
[roadrunner url]: https://github.com/valkyrjaio/valkyrja-roadrunner-php
[netty url]: https://github.com/valkyrjaio/valkyrja-netty-java
[jetty url]: https://github.com/valkyrjaio/valkyrja-jetty-java
[tomcat url]: https://github.com/valkyrjaio/valkyrja-tomcat-java
[docker url]: https://github.com/valkyrjaio/valkyrja-docker-php
[architecture url]: https://github.com/valkyrjaio/architecture
[org repos url]: https://github.com/valkyrjaio
[contributing url]: https://github.com/valkyrjaio/.github/blob/26.x/CONTRIBUTING.md
[vocabulary url]: https://github.com/valkyrjaio/.github/blob/26.x/VOCABULARY.md
[repository naming url]: https://github.com/valkyrjaio/.github/blob/26.x/REPOSITORY_NAMING.md
[security url]: https://github.com/valkyrjaio/.github/blob/26.x/SECURITY.md
