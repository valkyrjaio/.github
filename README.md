...
| [`_trailing-newline-check.yml`](.github/workflows/_trailing-newline-check.yml) | `workflow_call` | Reusable implementation of the above; skips binary files, checks PR-changed files (or all tracked files outside PR context); posts/removes a PR comment listing offending files on failure/success |
